...
## 0. 当前状态（agent/新会话先读这里，本节下方皆为历史规划）

- **当前分支**：zcode_bsp（基于 develop，参考 zcode 分支的重写）
- **最近完成**：接口驱动层重构修复（hotfix/interface-driver-refactor）、RocketPi 批次 1/2 大部分、LVGL 相册 Demo
- **待办批次**：批次 2 尾（pwm_motor 待 AT8236 资料；usb_cdc/usb_msc/mbedtls 建议单独排期）、批次 3（hcsr04/esp8266/irda/uart_radar）、批次 4（i2s 音频）
- **权威进度表**：`Docs/01-map/ARCHITECTURE.md` 第 8 节；已知问题清单在其第 9 节
- 本文件正文是历史规划存档,进度冲突时以 ARCHITECTURE.md 为准
...
4. **Project Manager -> Project**:
   ...
   - **Linker Settings**: `Minimum Heap Size` 设为 `0x1000` (4KB)，`Minimum Stack Size` 设为 `0x2000` (8KB)。这是因为引入 LVGL 图形子系统后，深层调用链和样式渲染计算需要足够的栈深度，以防发生栈溢出 (Stack Overflow) 触发 HardFault。
...
- `Docs/10-standards/代码规范.md`：项目代码规范
- `README.md`：模块状态表
...
- **SPI1**:
  ...
  - **DMA (SPI1_TX)**: 开启 `DMA2 Stream 3`, Channel 3, Mode `Normal`, Direction `Memory to Peripheral`, Data Width 必须配置为 `Byte` (因为 SPI 外设采用 8 Bits 数据位宽，配成 Half Word 会导致传输时数据跳字节与越界)。
- **SPI2 (W25Q128 Flash)**:
  - **模式**: `Full-Duplex Master` (引脚 `PB10` SCK, `PC2` MISO, `PC3` MOSI)。
  - **参数**: Data Size `8 Bits`, Prescaler `2`, CPOL `Low`, CPHA `1 Edge`。
- **TIM10 (屏幕背光)**:
  - **模式**: `PWM Generation CH1` (引脚 `PB8` 复用)。
  - **参数**: Prescaler=168-1, Counter Period (ARR) 设为 `99` (提供 100 阶占空比调节精度)，通道极性 (CH Polarity) 需配置为 `Low` 以匹配低电平有效的物理背光电路。
- **GPIO**: 
  - LCD 附属控制引脚 `PE14` (CS), `PD14` (DC), `PE1` (RST) 配置为 `GPIO_Output`，速度设为 `Very High`，初始输出电平必须设为 `High` (SET) 以在初始化期间实施总线隔离。
  - W25Q128 片选引脚 `PE4` (W25Q_CS) 配置为 `GPIO_Output`，速度设为 `Very High`，初始输出电平必须设为 `High` (SET)。
  - LCD 触摸中断引脚 `PE2` (TOUCH_INT) 配置为 `GPIO_EXTI2`，触发模式设为 `External Interrupt Mode with Falling edge trigger detection` (下降沿触发)，引脚内部上拉 `Pull-up`，并在 **NVIC** 中开启 `EXTI line2 interrupt` 中断，设为中等优先级。
- **Project Manager (堆栈配置)**:
  - **Minimum Stack Size** 必须调整为 `0x2000` (8KB)。
  - **Minimum Heap Size** 保持 `0x1000` (4KB)。
  - **【LVGL v9.3.0 堆栈及内存需求详解】**:
    - **栈空间 (Stack Size)**: LVGL 在运行时（特别是 v9 版本）涉及到非常深的多层函数嵌套调用（如对象事件分发、样式渲染树计算、小部件属性更新、绘图管线等）。在 32 位 ARM Cortex-M4 (STM32F407) 上，每次嵌套函数调用都会压栈，且局部变量也会占用栈空间。如果使用默认的 `0x1000` (4 KB) 栈大小，运行 GUI 交互或复杂界面时极易发生栈溢出 (Stack Overflow)，进而触发硬件中断异常 (HardFault)。因此，必须在 CubeMX 的 Project Manager 中将 **Minimum Stack Size** 配置为 **0x2000** (8 KB) 以上。
    - **堆空间 (Heap Size)**: 在 `lv_conf.h` 中，如果将 `LV_USE_STDLIB_MALLOC` 配置为内置的 `LV_STDLIB_BUILTIN`，它会使用 LVGL 自身的静态全局数组作为内部堆（由 `LV_MEM_SIZE` 宏定义，例如配置为 32 KB）。这种情况下，LVGL 的内部组件（控件创建、样式属性动态申请）不会直接消耗标准 C 库的堆（Heap），但如果应用层使用了动态内存分配，或者部分 C 库的格式化函数（如 `sprintf`/`sscanf` 等）在后台进行堆分配，仍然需要预留基本的堆空间。因此，CubeMX 的 **Minimum Heap Size** 保持 **0x1000** (4 KB) 即可；如果未来需要动态分配画布（Canvas）或选用系统堆分配（`LV_USE_STDLIB_MALLOC` 设为 `LV_STDLIB_CLIB`），则需将系统堆相应增加到 `0x2000` (8 KB) 或更大。
...
- `port_pwm.c/h`：通用 PWM 抽象层接口。
- `bsp_backlight.c/h`：屏幕背光 PWM 控制驱动，默认亮度为 50%。
- `dev_st7789.c/h`：提供阻塞式 `lcd_flush` 与 DMA 异步非阻塞式 `lcd_flush_async` / `lcd_flush_async_cb` 刷新接口，并在初始化中自动调用背光初始化。
- 注册 Shell 命令：`lcd_fill COLOR` / `lcd_test` / `backlight N`（0~100）
...
### M17 — LVGL 与 GUI\_Guider 移植 + 触摸集成
...
* **移植文件与目录结构**：
  * 将 `lvgl` 核心库及 NXP `GUI_Guider` 生成的 UI 代码集成至 `Middleware` 下，区分自动生成的 `generated` 与自定义逻辑的 `custom`。
* **`lv_port_disp.c` 移植设计（参考 v1 优秀实现）**：
  * **DMA 双缓冲零阻塞刷新**：配置大小为 `320 * 10` 的双缓冲区 `buf_2_1` 与 `buf_2_2`。在 `disp_flush` 中直接调用异步 DMA 刷新接口 `lcd_flush_async_cb`，当 DMA 完成传输产生中断时触发 `lv_flush_done_cb` 回调函数以通知 LVGL 释放缓冲区（调用 `lv_disp_flush_ready`），实现 CPU 渲染与 DMA 发送完全重叠的无感刷新。
  * **忙碌状态退避机制**：若 `lcd_flush_async_cb` 启动失败（如 DMA 忙碌或总线挂起），自动退避回退至阻塞式 `lcd_flush` 强制刷屏，确保界面绘制的安全与稳定性。
* **`lv_port_indev.c` 移植设计（参考 v1 优秀实现）**：
  * **低开销硬件中断引脚预判（关键优化）**：在调用 I2C 读取触摸控制器状态之前，先通过 GPIO 读取触摸芯片的物理中断引脚 PE2（`PORT_GPIO_TOUCH_INT`）。只有当 PE2 为低电平（表示有物理触摸事件）时才执行 I2C 读取；若为高电平则直接返回 `false`，从而将轮询时的 CPU 资源与 I2C 总线带宽占用降到最低。
  * **多向旋转屏幕坐标映射**：在 `touchpad_get_xy` 中，获取当前屏幕的物理旋转方向（`lcd_get_rotation()`），根据 0° / 90° / 180° / 270° 对读取的 12 位原始坐标进行旋转映射变换，确保屏幕上的 UI 交互位置与物理触摸点完全重合。
* **整机初始化与 Tick 关联**：
  * 在 SysTick 中断（`SysTick_Handler`）中调用 `lv_tick_inc(1)` 提供高精度心跳。
  * 在主循环（`app_main` / `bsp_board_process`）中持续调用 `lv_timer_handler()` 驱动定时任务与渲染引擎。
  * 在启动入口声明全局 `lv_ui guider_ui` 句柄，调用 `setup_ui(&guider_ui)` 和 `events_init(&guider_ui)` 挂载图形界面与交互事件。
* **验收条件**：运行 LVGL 官方精美音乐播放器 Demo (`lv_demo_music`)，或编译 GUI Guider 默认手表时钟 UI 主页，确保能通过触摸屏平滑进行卡片滑动与按钮交互。
...
* **实现细节**：
  * 提供一个基于 LVGL Canvas 或原生 LCD 画线接口的测试 Demo。
  * 手指按在屏幕上滑动时，在屏幕上实时跟随指尖绘制出平滑的触摸轨迹。
  * 支持按下开发板板载按键（KEY）清屏或通过 Letter Shell 发送命令清空当前画布。
* **验收条件**：屏幕轨迹连贯无断块，多点轨迹解析正常，且清屏逻辑响应即时。
...
- **EXTI**: 配置传感器中断引脚 (如 SD3078\_INT PE3) 为外部中断输入，并在 NVIC 开启响应线。*(注：板载六轴姿态传感器 ICM-42688-P 的中断引脚在硬件上悬空，仅能通过轮询方式读取姿态数据)*
- **GPIO**: 必须在 CubeMX 中将 **PE7** (IMU_CS) 配置为 **GPIO_Output**（推挽输出、无上下拉、最高速度、初始电平为 **High**），作为姿态传感器的 SPI 片选引脚。
...
### M22 — AT24C02 EEPROM (已在 M12 提前完成)
- 仅保留预留位或直接跳过
...
- **TIM4 (Encoder Mode)**: 启用 **TIM4**，并将 **Combined Channels** 配置为 **Encoder Mode**。确保物理引脚 **PD12** 复用为 **TIM4_CH1**，**PD13** 复用为 **TIM4_CH2**（双通道 4 倍频计数）。在参数设置中，建议开启输入通道的硬件滤波（如 **Filter = 15**）以抑制机械抖动，**Counter Period (ARR)** 设为 **65535**。
- **ADC1**: 开启片上内置 **ADC1** 外设，配置 **IN10**（对应电位器引脚 **PC0**）为单通道轮询采样模式；同时可开启 **IN8** (PB0_PRE 复用引脚) 与 **IN9** (PB1_PRE 复用引脚) 以备后续外部 ADC 接口扩展。
...
- `dev_ec11.c/h`：基于 **TIM4** 硬件正交解码模式（Encoder Mode）获取旋转方向及步数。
- Shell 命令：`ec11_monitor`（输出旋转事件）

***

### M26 — 内置 ADC 接口与旋转电位器驱动

```
feat(interface): port_adc - 内置 ADC 接口层实现
feat(driver):    dev_potentiometer - 旋转电位器驱动
```

- `port_adc.c/h`：封装 HAL 库的 ADC 轮询采样逻辑，向驱动层暴露统一的电压读取（单位 mV）或原始 LSB 采样接口。
- `dev_potentiometer.c/h`：实现电位器设备驱动，完成物理电压到旋转百分比（0.0% ~ 100.0%）的映射解算。
- 注册 Shell 命令：`adc_read`（读取指定通道电压及 LSB）/ `pot_read`（显示电位器旋转开度比）。

***

### M27 — HX711 24 位 ADC（称重传感器）

```
feat(driver): dev_hx711 - 24-bit Delta-Sigma ADC 驱动
```

- 备注：HX711 与称重传感器本身属于外部扩展物理模块，使用板上预留的外接引脚排针进行物理连接。
...
**验收**：
- 在 Shell 终端触发命令，旋转电位器 RV1，其开度在 0.0% ~ 100.0% 之间顺滑递增，无断点和剧烈抖动。
- 电子秤能够准确读数并实时回显至屏幕上。

## 阶段七：高级数据传输与 GUI 应用（3 提交）

**@\[CubeMX配置]**

- **Project Manager**: LVGL 及相关 UI 框架需要极大的堆栈空间。请确保 `Minimum Stack Size` 维持为 `0x2000` (8KB)，若在应用层动态创建大量控件或分配大型画布，需要将 `Minimum Heap Size` 对应增加到 `0x2000` (8KB) 甚至更高。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 2.4.0 | 2026-06-21 | 增补 PE2 触摸中断引脚 CubeMX 配置规范要求与 FT6336 驱动中断化改造实现 |
| 2.3.0 | 2026-06-11 | 移除 M16 裸机文本自检；将触摸与 LVGL 移植提前至阶段四；因暂无资源，将电机控制（原阶段八）后移至以太网后面（新阶段十一） |
