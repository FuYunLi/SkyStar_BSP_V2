...
- **当前分支**：zcode_bsp（基于 develop，规范化开发线）
- **最近完成**：接口驱动层重构修复（hotfix/interface-driver-refactor）、阶段 7 性能与内存优化
- **待办**：由用户在会话中提出；确定后登记于 `Docs/01-map/ARCHITECTURE.md` 第 8 节任务台账
- **权威进度表**：`Docs/01-map/ARCHITECTURE.md`；已知问题清单在其第 9 节
