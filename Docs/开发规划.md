...
- **GPIO/EXTI (Encoder A/B)**: 将 **PB0** 与 **PB1** 配置为普通的 GPIO_Input 或 EXTI 外部中断引脚。由于 PB0/PB1 在 STM32F407 上对应 TIM3_CH3/CH4，而硬件编码器接口仅支持定时器的 CH1/CH2，因此必须使用软件正交解码（可采用 1ms 定时器轮询或外部中断触发）。
...
- `dev_ec11.c/h`：旋转方向 + 步数，基于 EXTI 外部中断与正交状态机解码。
