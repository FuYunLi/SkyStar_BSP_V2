...
**验收**：
- I2C 与 SPI 对外暴露的核心传输 API 在参数形式上具备高度一致性，底层具备死锁解救机制。
...
- **GPIO**: LCD 附属控制引脚 `PE14` (CS), `PD14` (DC), `PE1` (RST) 配置为 `GPIO_Output`，速度设为 `Very High`。为 FT6336 的软件 I2C PD10(SCL),PE13(SDA)预留的引脚配置为 `GPIO_Output` (设为 `Open-Drain` 开漏输出，速度 `High` 更稳定)。
